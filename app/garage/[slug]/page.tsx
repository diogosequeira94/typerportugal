...
type Build = {
  ...
  instagram: string | null; facebook: string | null; location: string | null;
  year: string; colour: string; power: string; torque: string; transmission: string;
  status: string; note: string; parts: string[][]; gallery: { src: string; alt: string }[];
};

const builds: Record<string, Build> = {
  fk8: {
    code: "FK8", name: "Civic Type R FK8", title: "O PADRÃO BRANCO.", image: "/cars/diogo/fk8-03.webp", owner: "Membro fundador", instagram: "diogo.arcseq", facebook: null, location: "Centro (Lisboa)", year: "2019", colour: "Championship White", power: "320 CV", torque: "400 Nm", transmission: "6 MT", status: "Configuração de fábrica", note: "Um FK8 imaculado, anterior ao facelift, mantido exatamente como a Honda o idealizou — o ponto de partida para um projeto que dispensa apresentações.",
  ...
  fk2: {
    code: "FK2", name: "Civic Type R FK2", title: "A PRIMEIRA ERA TURBO.", image: "/cars/fk2.jpg", owner: "Destaque da garagem", instagram: null, facebook: null, location: null, year: "2016", colour: "Branco Championship", power: "310 CV", torque: "400 Nm", transmission: "6 MT", status: "Destaque da comunidade", note: "O Type R turbo, puro e determinado, que redefiniu o ritmo para uma nova era de desempenho Civic.",
  ...
  fl5: {
    code: "FL5", name: "Civic Type R FL5", title: "O PRÓXIMO ÁPICE.", image: "/cars/fl5.jpg", owner: "Destaque da garagem", instagram: null, facebook: null, location: null, year: "2023", colour: "Branco Championship", power: "329 CV", torque: "420 Nm", transmission: "6 MT", status: "Destaque da comunidade", note: "Uma silhueta Type R mais limpa e madura, com a mesma dedicação à tração dianteira no seu ADN.",
...
export default async function BuildPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let build = builds[slug];

  if (!build && isSupabaseConfigured()) {
    ...
    if (car) {
      ...
      build = {
        code: car.generation, name: car.model, title: `${car.generation} DE ${car.owner_name}.`,
        image: car.cover_image_url || gallery[0]?.src || "/cars/fk8.jpg", owner: car.owner_name,
        instagram: car.instagram, facebook: car.facebook, location: car.location, year: String(car.year),
  ...
  if (!build) notFound();
  const facebookUrl = build.facebook && /^https:\/\/(www\.)?facebook\.com\//i.test(build.facebook) ? build.facebook : null;
