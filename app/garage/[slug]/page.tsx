...
export default async function BuildPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="profile-page">
    ...
    <section className="profile-content">
      <aside className="owner-card"><p className="eyebrow"><span /> Proprietário</p><h2>{build.owner}</h2><p>Parte da comunidade Type R Garage Portugal.</p><a href="https://instagram.com/" target="_blank" rel="noreferrer"><b>◎</b> Instagram <span>{build.handle}</span></a><a href="https://facebook.com/" target="_blank" rel="noreferrer"><b>f</b> Facebook <span>Type R Garage PT</span></a>{build.location && <div className="owner-location"><b aria-hidden="true">⌖</b><span><small>Localização</small>{build.location}</span></div>}<div className="side-gallery"><p className="eyebrow"><span /> Galeria do proprietário</p><figure style={{ backgroundImage: `url(${build.image})` }}><figcaption>Abrir galeria</figcaption></figure></div><div className="side-sound"><p className="eyebrow"><span /> Som do escape</p><b>▶</b><div><strong>Ouvir o carro</strong><span>A aguardar vídeo do proprietário</span></div></div></aside>
